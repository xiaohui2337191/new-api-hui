	...
	common.RelayCtxGo(ctx, func() {
		...
		for scanner.Scan() {
			...
			data := scanner.Text()
				...
				// 调试日志：记录扫描到的每一行
			if len(data) > 0 && common.DebugEnabled {
				logger.LogDebug(c, "[DEBUG] Scanner received: %s", data)
			}
					...
					if strings.HasPrefix(data, "data:") {
						...
						if data == "" {
							continue
										...
										// 可能是非流式响应（JSON 对象），也传递处理
										logger.LogDebug(c, "[DEBUG] Non-SSE response detected: %s", data)
									} else {
										// 其他格式，跳过
										continue
									}
									
									if !strings.HasPrefix(data, "[DONE]") {				info.SetFirstResponseTime()
