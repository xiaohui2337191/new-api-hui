...
func DoApiRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*http.Response, error) {
	...
	common2.SysLog(fmt.Sprintf("DoApiRequest: fullRequestURL=%s, ChannelBaseUrl=%s", fullRequestURL, info.ChannelBaseUrl))
...
func doRequest(c *gin.Context, req *http.Request, info *common.RelayInfo) (*http.Response, error) {
	// 增加活跃请求计数，阻止自动测速
	helper.IncrementActiveRequests()
	defer helper.DecrementActiveRequests()
	
	var client *http.Client
	var err error
	
	// 使用指纹随机化客户端（AirForce、DeepInfra等需要绕过检测的渠道）
	if helper.ShouldUseFingerprintClient(info.ChannelBaseUrl) {
		client = service.GetAirforceClient()
		if client == nil {
			common2.SysLog("Fingerprint client is nil, falling back to default client")
			client = service.GetHttpClient()
		} else {
			common2.SysLog("Using fingerprint client (TLS randomization + DisableKeepAlives=true)")
			// 设置随机UA
			ua := helper.GetRandomUserAgent()
			lang := helper.GetRandomAcceptLanguage()
			req.Header.Set("User-Agent", ua)
			req.Header.Set("Accept-Language", lang)
			
			// 如果启用了Clash代理池，切换节点
			if helper.IsClashEnabled() {
				node, err := helper.RotateClashNode()
				if err != nil {
					common2.SysLog(fmt.Sprintf("Clash节点切换失败: %v", err))
				} else {
					common2.SysLog(fmt.Sprintf("已切换Clash节点: %s", node))
				}
			}
		}
	...
	if err != nil {
		logger.LogError(c, fmt.Sprintf("do request failed: %v (url=%s, method=%s)", err, req.URL, req.Method))
		return nil, types.NewError(err, types.ErrorCodeDoRequestFailed, types.ErrOptionWithHideErrMsg(fmt.Sprintf("upstream error: %v", err)))
	}
	if resp == nil {
