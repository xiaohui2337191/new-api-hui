...
func (a *Adaptor) DoRequest(c *gin.Context, info *relaycommon.RelayInfo, requestBody io.Reader) (any, error) {
	// AirForce全局限流: 使用TokenBucket平滑限流
	if helper.IsAirForceChannel(info.ChannelBaseUrl) {
		common.SysLog(fmt.Sprintf("AirForce channel detected, base_url=%s", info.ChannelBaseUrl))
		
		// 从 context 获取 channel 的 keys 数量
		channelInterface, exists := c.Get("channel")
		if exists {
			if channel, ok := channelInterface.(*model.Channel); ok && channel != nil {
				keys := channel.GetKeys()
				keyCount := len(keys)
				
				if keyCount > 0 {
					// 获取下一个可用key（用于tool calls连续请求时轮换key）
					limiter := helper.GetAirForceRateLimiterV2()
					nextKey := limiter.GetNextAvailableKey(keyCount, info.ChannelMeta.ChannelMultiKeyIndex)
					
					if nextKey < 0 {
						// 所有key都在冷却中，等待并尝试获取
						logger.LogWarn(c, "All AirForce keys are in cooldown, waiting...")
						time.Sleep(3 * time.Second)
						nextKey = limiter.GetNextAvailableKey(keyCount, info.ChannelMeta.ChannelMultiKeyIndex)
						if nextKey < 0 {
							return nil, fmt.Errorf("rate limit: all AirForce keys are rate limited (RPM: 1 request per 60 seconds per key)")
						}
					}
					
					// 更新到下一个key
					if nextKey != info.ChannelMeta.ChannelMultiKeyIndex {
						info.ChannelMeta.ChannelMultiKeyIndex = nextKey
						info.ChannelMeta.ApiKey = keys[nextKey]
						common.SysLog(fmt.Sprintf("AirForce switched to key index %d", nextKey))
					}
					
					// 存储key索引用于响应后标记
					c.Set("airforce_key_index", nextKey)
				}
			}
		}
		...
		if helper.IsClashEnabled() {
			// 检查是否是全局限流重试（不换节点）
			if isGlobalRetry, exists := c.Get("airforce_global_rate_limit_retry"); exists && isGlobalRetry.(bool) {
				// 全局限流重试：保持当前节点，不切换
				common.SysLog("AirForce全局限流重试: 保持当前节点")
				// 清除标记
				c.Set("airforce_global_rate_limit_retry", false)
			} else if shouldSwitch, exists := c.Get("airforce_retry_switch_node"); exists && shouldSwitch.(bool) {
				// key rate limit：必须切换节点
				common.SysLog("AirForce key rate limit: 切换到新节点")
				if nodeName, err := helper.EnsureAvailableNode(); err != nil {
					common.SysLog(fmt.Sprintf("智能选择无可用节点: %v", err))
				} else {
					common.SysLog(fmt.Sprintf("智能选择当前节点: %s", nodeName))
				}
				// 清除标记
				c.Set("airforce_retry_switch_node", false)
			} else {
				// 正常请求：检查当前节点是否冷却，如果是则切换
				if nodeName, err := helper.EnsureAvailableNode(); err != nil {
					common.SysLog(fmt.Sprintf("智能选择无可用节点: %v", err))
				} else {
					common.SysLog(fmt.Sprintf("智能选择当前节点: %s", nodeName))
...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.NewAPIError) {
	...
	// AirForce 渠道：响应成功后标记节点冷却（避免重试时切换节点）
	if err == nil && helper.IsAirForceChannel(info.ChannelBaseUrl) && helper.IsClashEnabled() {
		helper.MarkCurrentNodeShortCooldown()
		common.SysLog(fmt.Sprintf("AirForce 请求成功，节点 %s 已标记冷却", helper.GetCurrentClashNode()))
	}
